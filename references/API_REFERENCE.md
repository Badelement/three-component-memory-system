# Three-Component Memory System - API Reference
...
**Example:**
```python
# Called automatically by OpenClaw integration
...
memory._cache.clear()  # Clear cache
```
...
```python
# Enable test mode
memory = MemorySystem({"test_mode": True})

# Mock data
memory._test_add_mock_memory("Test memory content")
```

### Benchmarking
```python
# Run performance tests
results = memory.benchmark_search(iterations=100)
print(f"Average search time: {results['avg_time']}ms")
```
